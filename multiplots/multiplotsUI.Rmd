...
imgFolder <- file.path(.root(),"img")
```

```{r}
xfun::download_file("https://www.dropbox.com/s/atckvqnb7e0wre3/multipleplots_drake.Rdata?dl=1", mode="wb")
load("multipleplots_drake.Rdata")
.root <- rprojroot::is_rstudio_project$make_fix_file()
drake_multipleplots$.updateCache()
drake_multipleplots$source_plan()
drake_multipleplots$makePlan()
```
...
```{r}
drake_multipleplots$loadTarget$subplot_p5()
drake_multipleplots$loadTarget$plt_leaflet()
save(
  subplot_p5, plt_leaflet,
  file="multiplots.Rdata"
)
```

```{r makecondition_mp}
load("multiplots.Rdata")
```

```{r tag_mixPlots}
tag_mixPlots <- htmltools::withTags(
  div(
    class="row",
    div(
      class="col s6",
